...
export class TodoListImageController {
  ...
  async find(@param.filter(TodoListImage) filter?: Filter<TodoListImage>): Promise<TodoListImage[]> {
  ...
  async replaceById(@param.path.number('id') id: number, @requestBody() todoListImage: TodoListImage): Promise<void> {
  ...
  async getTodoList(@param.path.number('id') id: typeof TodoListImage.prototype.id): Promise<TodoList> {
